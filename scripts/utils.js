...
const existsDllLibrary = env =>
  fs.existsSync(resolveApp(`${paths.appStatic}/${systemPath.appdllLibrary}_${env}.dll.js`));
...
class AsyncHook {
  ...
  callAsync(...args) {
    ...
    const next = result => {
      this.hooks[index]
        ? this.hooks[index]['fn'](this.hooks[index]['name'], task, result, next)
        : done(task, result);
      index++;
    };
    next();
